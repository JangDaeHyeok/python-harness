...
- 패키지 경로가 필요한 곳(구조 게이트·Phase·modify·템플릿)은 `package` 대신 `policy.package_dir`를 쓴다(flat·src 공통 경로). 상세는 ADR-0014.
- modify 검증 요약은 `policy.commands.lint`/`type`를 `shlex.split` 후 `run_argv_safe`로 실행한다. 정책 없으면 `ruff check .`/`mypy <package_dir>` 폴백, allowlist 밖 명령은 안전 생략.
...
## 관련 ADR
- 0006 체크포인트와 재개, 0008 수정 모드와 프로젝트 정책, 0009 Phase 실행과 컨텍스트 격리, 0010 고정 구조 강제, 0014 src 레이아웃과 source_root.
