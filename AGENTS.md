...
6. **print 금지** **[hook]**: `harness/` 내부에서 `print()` 사용 금지, logging 사용. 본 저장소는 `guard_no_print.py` PreToolUse 훅이 즉시 차단하고, `harness-init`으로 생성/마이그레이션되는 외부 프로젝트는 `harness_structure.yaml`의 `no_print_debug` error 규칙이 구조 검사 실패로 강제한다.
...
7. 반영 커밋을 push한다.
8. `--confirm-github-writes`가 있으면 원본 리뷰 코멘트에 한국어 답글을 남긴다.
9. `--auto-merge --confirm-github-writes`가 있으면 PR을 머지한다.
...
- ACCEPT 코멘트를 반영한 뒤 `--confirm-github-writes`가 명시된 경우에만 원본 코멘트에 한국어 답글을 남긴다.
