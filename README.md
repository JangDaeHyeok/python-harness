...
harness pr                      # 4) 변경을 PR로 올리고 리뷰 반영
```
...
| 변경을 PR로 올리고 리뷰 반영하기 | `harness pr` | `auto-pr-pipeline` |
...
마이그레이션은 루트 패키지(`my_package/__init__.py`)와 `src/<패키지>` 레이아웃을 모두 자동 탐지한다. 후보가 하나면 자동 채택하고 정책에 `project.source_root`까지 기록한다(ADR-0014). 후보가 여러 개일 때만 중단하고 `.harness/project-policy.yaml`의 `project.package`를 명시하라고 안내한다. 정책에 `package`/`source_root`가 이미 있으면 명시 값이 자동 탐지보다 우선하므로, 루트와 `src/`에 같은 패키지가 함께 있어도 명시한 위치로 고정된다.
...
9. 반영 커밋 push
10. (`--pr-confirm-github-writes`가 있으면) 원본 코멘트에 한국어 답글
11. (`--pr-auto-merge --pr-confirm-github-writes`가 있으면) PR 머지
```
...
이미 열린 PR 리뷰만 다시 처리하려면 `harness --auto-pr`에서도 새 PR 생성을 건너뛸 수 있다.

```bash
harness --mode modify --auto-pr --pr-number 123 --pr-no-poll "리뷰 코멘트를 반영해주세요"
harness --mode modify --auto-pr --pr-current-pr --pr-no-poll "현재 PR 리뷰를 반영해주세요"
```
...
이 한 줄이 push → PR 생성 → 리뷰 수집 → ACCEPT 코멘트 반영 → 한국어 답글 → 머지까지 자동 수행한다. 머지 직전에 멈추고 싶으면 `--pr-auto-merge`만 뺀다. 답글도 남기지 않으려면 `--pr-confirm-github-writes`를 빼면 된다.
...
harness --mode modify --use-headless-phases --auto-pr --pr-base main "..."  # End-to-End(답글/머지 제외)
