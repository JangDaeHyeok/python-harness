...
|-------------|----------------|----------|
| 시작 전 환경이 준비됐는지 확인하기 | `harness-doctor` | git·`gh` 인증·ruff/mypy/pytest 설치·정책 파일이 갖춰졌는지 한 번에 점검할 때 |
| 새 프로젝트에 하네스 규칙 파일만 깔기 | `harness-init --offline "프로젝트 설명"` | ADR, 컨벤션, 구조 규칙, CLAUDE.md, `.claude` 설정을 처음 배치할 때 |
| 새 프로젝트에 Python 골격까지 함께 깔기 | `harness-init --offline --scaffold "프로젝트 설명"` | 위 규칙 파일에 더해 `pyproject.toml`·`.gitignore`·CI·smoke 테스트 골격이 필요할 때 |
| 기존 Python 프로젝트를 하네스 구조로 보강하기 | `harness-init --migrate --offline "프로젝트 설명"` | 이미 코드가 있는 저장소(`src/` 레이아웃 포함)에 누락된 `docs/`, `.harness/`, `tests/`, `scripts/` 등을 맞출 때 |
...
| 이미 있는 PR의 리뷰 코멘트만 처리하기 | `auto-pr-pipeline --current-pr --no-poll` | 현재 브랜치의 PR 리뷰를 수집하고 ACCEPT 코멘트를 반영하되, 답글/머지는 승인 전 건너뛸 때 |
