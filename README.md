...
- `harness fix`는 현재 디렉터리를 **수정(modify) 모드**로 고친다. 작은 수정은 그대로 동작하며, 큰 작업은 `harness fix --headless "..."`로 Phase 단위 실행을 켤 수 있다.
- `harness ship`은 **수정 → 헤드리스 Phase 구현 → push → PR 생성 → 리뷰 반영**까지 한 번에 묶는다. 자동 머지·리뷰 답글은 팀 공유 권한 밖 GitHub 쓰기라 `--pr-auto-merge --pr-confirm-github-writes`를 명시할 때만 동작한다.
- `harness review`는 현재 브랜치에 연결된 PR의 리뷰 코멘트만 다시 처리한다.
- `harness init --offline`은 LLM 호출 없이 기본 하네스 파일을 깐다. 자연어 의도로 더 보강하려면 `HARNESS_API_ENDPOINT` 설정 후 `--offline`을 빼면 된다.
- 이 별칭들은 기존 명령(`harness --mode modify`, `harness-init`, `harness-doctor`, `auto-pr-pipeline`)을 그대로 감싼 것이다. 세부 옵션과 고급 전략 플래그는 [docs/operations.md](docs/operations.md)에 정리되어 있다.
...
처음이라면 [빠른 시작](#빠른-시작)의 4개 별칭으로 충분하다. 아래 표는 별칭과 그에 대응하는 전체 명령, 그리고 상황별 선택지를 정리한 것이다. 고급 전략 플래그(`--use-headless-phases`, `--allow-empty-docs-diff`, `--headless-phase-timeout`)는 [docs/operations.md](docs/operations.md)에 따로 모아두었다.
...
별칭으로 표현하기 어려운 세부 시나리오(오프라인 부트스트랩, 골격 생성, 마이그레이션, 기존 PR 번호 지정, PR 본문만 생성 등)는 아래 [시나리오별 CLI 레시피](#시나리오별-cli-레시피)와 [docs/operations.md](docs/operations.md)를 참고한다. 별칭 뒤에는 대응 명령의 모든 플래그를 그대로 붙일 수 있다(예: `harness fix --auto-pr --pr-base main "..."`).
...
`harness`는 다시 초보자용 서브커맨드 `doctor / init / fix / ship / pr / review`를 흡수한다. 각각 `harness-doctor`·`harness-init`·`harness --mode modify`(+ PR 자동화)·`auto-pr-pipeline`로 위임하는 별칭이라, entrypoint 개수(5개)와는 별개다.
...
빈 디렉터리에서 시작해 PR 리뷰 답글·머지까지 도달하는 가장 흔한 경로를 한 시나리오로 잇는다. 각 단계는 앞 단계의 산출물을 그대로 사용하므로 위에서부터 차례로 따라가면 된다. 특정 작업만 빠르게 찾고 싶으면 아래 [시나리오별 CLI 레시피](#시나리오별-cli-레시피)를 본다.
...
CodeRabbit 리뷰까지 함께 쓰려면 `--with-coderabbit`로 `.coderabbit.yaml`을 같이 배포한다(설정 파일만 만들며, GitHub 저장소의 CodeRabbit App 설치·권한 승인은 별도).
...
또는 셸에서 직접. 별칭 `harness ship`이 가장 짧고, 전체 옵션을 직접 통제하려면 풀 명령을 쓴다(둘은 동일한 파이프라인이다):
...
`--auto-pr`로 이어지는 전체 파이프라인(머지·답글 플래그 포함)의 단계:
...
> 구현이 실패하면(통과 스프린트 0개) PR 단계를 건너뛴다. PR 파이프라인 실패는 구현 결과에 영향을 주지 않는다. CI 게이트에서 PR 실패까지 잡으려면 `--fail-on-pr-error`를 더한다.
...
PR이 열린 뒤 사람·CodeRabbit이 코멘트를 남기면, 같은 명령에 `--pr-confirm-github-writes --pr-auto-merge`를 더해 마지막까지 자동으로 갈 수도 있고, 별도로 분리해 처리할 수도 있다.
...
5. `--confirm-github-writes` 또는 `--pr-confirm-github-writes`가 명시된 경우 원본 코멘트에 한국어 답글
...
머지까지 한 줄로 가고 싶었으면 3단계에서 이미 `--pr-auto-merge --pr-confirm-github-writes`를 줬을 것이고, 그렇지 않다면:
...
`harness-init`만 한 번 깔아두면, 이후 모든 변경은 위 한 줄(또는 자연어 한 문장)로 init·구현·PR·리뷰 반영·머지가 일관된 파이프라인으로 이어진다. 단, 리뷰 답글(`gh api --method POST`)과 머지(`gh pr merge`)는 팀 공유 permission 밖 작업이라 확인 플래그가 붙은 실행은 Claude Code에서 별도 승인을 거친다.
...
이 한 줄이 push → PR 생성 → 리뷰 수집 → ACCEPT 코멘트 반영 → 한국어 답글 → 머지까지 자동 수행한다. 머지 직전에 멈추고 싶으면 `--pr-auto-merge`만 뺀다. 답글도 남기지 않으려면 `--pr-confirm-github-writes`를 빼면 된다. 머지·답글을 뺀 기본 흐름은 별칭 `harness ship "..."`으로 더 짧게 쓸 수 있다(필요하면 `harness ship --pr-auto-merge --pr-confirm-github-writes "..."`).
...
modify/resume, PR 본문 생성, PR 자동화 시작 전에는 고정 구조 게이트도 실행된다. 필수 경로(`docs/`, `docs/adr/*.md`, `docs/code-convention.yaml`, `harness_structure.yaml`, `.harness/project-policy.yaml`, `tests/`, `scripts/`, 정책의 패키지 디렉터리)가 없으면 실행을 중단하고 `harness-init --migrate`를 안내한다. 패키지 디렉터리는 정책의 `project.package`와 `project.source_root`를 합친 경로(flat은 `<패키지>`, src 레이아웃은 `src/<패키지>`)로 확인한다 (ADR-0010, ADR-0014).
...
`scripts/auto_pr_pipeline.py`는 push → PR → 리뷰 수집 → 분류 → ACCEPT 반영 순서로 기본 동작한다. 답글(`gh api --method POST`)과 선택적 머지(`gh pr merge`)는 `--confirm-github-writes`가 명시된 경우에만 실행한다.
...
외부 프로젝트나 신규 저장소에 하네스 규칙 묶음을 한 번에 배치한다. ADR/컨벤션/구조/정책/CLAUDE.md와 `.claude` 팀 설정 + 안전한 Stop 훅 + LLM 보강 + 템플릿 폴백 + dry-run을 지원한다. 생성되는 `no_print_debug` 구조 규칙은 `severity: error`라서 외부 프로젝트에서도 `print()` 디버깅은 구조 검사 실패로 이어진다.
...
기존 Python 프로젝트에는 `harness-init --migrate`를 사용한다. 이 경로는 ADR/컨벤션/구조/정책만 대상으로 삼고, 단일 Python 패키지를 정책의 `project.package`에 고정한다. 루트 패키지뿐 아니라 `src/<패키지>` 레이아웃도 자동 인식해 정책에 `project.source_root: src`를 기록한다 (ADR-0014). 패키지 후보가 여러 개라 자동 선택이 위험한 경우에는 명확한 오류로 멈추고 정책에 명시할 패키지를 요구한다 (ADR-0011).
...
| **부트스트랩 배포 채널** | `harness-init`이 위 셋업을 함께 배포 | 외부 프로젝트가 첫날부터 동일 검증 환경 |

**의미**: CRITICAL 규칙이 "모델이 기억해야 하는 것"에서 "시스템이 강제하는 것"으로 이동했다. 예) 본 저장소의 `harness/`에 `print(` 작성 시도는 hook이 즉시 차단(exit 2), 외부 프로젝트의 `print()` 디버깅은 `no_print_debug` error 구조 규칙으로 실패, 세션 종료 시 ruff/mypy/structure 자동 실행, ADR/리뷰 작업 시 skill 형식 자동 로드.
...
|------|------|----------|--------------------|
| 팀 공유 permission | `.claude/settings.json` | 세션 시작 시 | ruff/mypy/pytest/`gh pr view·list·diff·status·checks·comment·create` 등이 매번 승인을 묻지 않고 바로 실행됨. `gh api` 쓰기와 `gh pr merge`는 확인 플래그 실행이 deny에 걸려 별도 승인이 필요 |
...
**무엇을 해주는가**
PR에 달린 사람·CodeRabbit 코멘트를 `ACCEPT` / `DEFER` / `IGNORE`로 분류하고, ACCEPT만 코드에 반영한다. 한국어 답글 작성은 `--confirm-github-writes` 또는 `--pr-confirm-github-writes`가 명시된 승인 흐름에서만 수행한다.
...
| `--auto-pr` 실행이 PR 단계 진입 직전에 멈춤 | `gh` CLI 미인증 또는 미설치 | `gh auth login` 수행 후 재실행. 시작 전 `harness-doctor`로 git·`gh`·도구·정책 준비 상태를 한 번에 점검할 수 있다 |
| `--auto-pr`가 구현은 성공했는데 PR이 실패했고 종료 코드가 0이라 CI가 놓침 | PR 파이프라인 오류는 기본적으로 종료 코드에 반영되지 않음 | `--fail-on-pr-error`를 추가하면 PR 오류 시 종료 코드 1. PR 결과는 `.harness/artifacts/auto-pr-result.json`에도 기록된다 |
